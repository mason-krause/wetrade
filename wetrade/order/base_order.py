...
class BaseOrder:
    ...
    self.preview_order_request = {
      'PreviewOrderRequest': {
        ...
        'Order': {
          ...
          'orderTerm': order_term, # [GOOD_UNTIL_CANCEL, GOOD_FOR_DAY, GOOD_TILL_DATE, IMMEDIATE_OR_CANCEL, FILL_OR_KILL]
  ...
  def check_status(self):
    ...
    response, status_code = self.client.request_order_detail(account_key=self.account_key, order_id=self.order_id, symbol=self.symbol)
    status = ''
    execution_price = 0.0
    with suppress(Exception):
